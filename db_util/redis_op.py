...
redis_config = Config()
host = redis_config.redisHost
pwd = redis_config.redisPassword
port = redis_config.redisPort
...
class RedisUtil:
    ...
    def __init__(self, host=host, pwd=pwd, port=port, db_index=0):
        self.host = host
        self.pwd = pwd
        self.port = port
        self.db_index = db_index
        self.pool = redis.ConnectionPool(host=self.host, password=self.pwd, port=self.port, db=self.db_index)
    ...
    # 设置过期时间，单位秒
    def string_set_ex(self, key, value, time=600):
        return self.r.setex(key, value=value, time=time)

    # 设置过期时间，单位毫秒
    def string_set_px(self, key, value, time_ms):
    ...
    def hash_hget(self, name, key):
        return self.r.hget(name, key).decode()
    ...
    # 获取name中所有的values
    def hash_get_values(self, name):
        return self.r.hvals(name)

    # 检查传入的key是否存在于name的hash中
    def check_hash_isexisted(self, name, key):
