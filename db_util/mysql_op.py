...
from utils.data_structure.lists import ListUtils
from utils.date_time_util.date_time import DateTimeUtil
from utils.log import log

extract_column_from_sql_re = re.compile(r"^select.+from", re.I)
...
DEFAULT_DB_CONNECTION_CONFIG = MysqlConnectionConfig(
    "localhost", "root", "123", "test_plat", 3306)
...
def get_query_sql(table_name: str
    ...
    if not table_name:
        raise RuntimeWarning("要查询的表名不能为空!")
    if column:
        if distinct:
...
def parse_query_result(query_result: list
    ...
    if result_formatter:
        ...
        for res in query_result:
            new_res = DateTimeUtil.formatter_line(res, datetime_formatter)
            if "tuple" == result_type:
                ...
                res_dict = ListUtils.list2dict(column, new_res)
                new_query_result.append(res_dict)
            else:
                error_msg = "不支持当前操作数 %s 或操作数为 dict 时, column为空" % result_type
...
def query(cursor: Cursor
    ...
    if write_type not in ["dict", "list", "tuple"]:
        error_msg = "不支持当前操作数 %s" % write_type
        raise RuntimeWarning(error_msg)
    ...
    if sql:
        ...
        if column_match:
            ...
            msg = "query_sql可能不被支持, 请检查 %s" % query_sql
    ...
    if is_only_print_sql:
        ...
        try:
            ...
            log.error("查询出错,使用的sql是: %s" % query_sql)
            raise e
...
def get_update_sql(table_name: str, item_list: list, optional=None, condition: dict = None,
    ...
    if not table_name or not item_list:
        raise RuntimeWarning("表名和数据对象不能为空!")
    ...
    if not optional or "Insert" == optional:
        ...
        return "insert into %s (', '.join(%s)) values (', '.join(%s))" % (table_name, columns, values)
...
def insert(cursor: Cursor, table_name: str, item: dict, is_only_print_sql: bool = False):
    ...
    if is_only_print_sql:
        log.info(f"使用的插入语句是：{insert_sql}")
    else:
        try:
            log.debug(f"使用的插入语句是：{insert_sql}")
            num = cursor.execute(insert_sql)
            log.info(f"insert 影响的行数是 {num}")
            return cursor.lastrowid
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"插入出错,使用的sql是: {insert_sql}")
            raise e
...
def batch_insert(cursor: Cursor, table_name: str, item_list: list, is_only_print_sql: bool = False):
    ...
    if is_only_print_sql:
        log.info(f"使用的批量插入语句是：{batch_insert_sql}")
    else:
        try:
            log.debug(f"使用的批量插入语句是：{batch_insert_sql}")
            num = cursor.execute(batch_insert_sql)
            log.info(f"batch_insert 影响的行数是 {num}")
            return num
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"批量插入出错,使用的sql是: {batch_insert_sql}")
            raise e
...
def update(cursor: Cursor, table_name: str, item: dict, condition: dict, is_only_print_sql: bool = False,
    ...
    if is_only_print_sql:
        log.info(f"使用的更新语句是：{update_sql}")
    else:
        try:
            log.debug(f"使用的更新语句是：{update_sql}")
            num = cursor.execute(update_sql)
            log.info(f"update 影响的行数是 {num}")
            return cursor.lastrowid
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"更新出错,使用的sql是: {update_sql}")
            raise e
...
def batch_update(cursor: Cursor, table_name: str, item_list: list, condition: dict, is_only_print_sql: bool = False,
    ...
    batch_update_sql = get_update_sql(table_name, item_list, optional="BatchUpdate", condition=condition,
                                      update_key_prefix=update_key_prefix)
    if is_only_print_sql:
        log.info(f"使用的批量更新语句是：{batch_update_sql}")
    else:
        try:
            log.debug(f"使用的批量更新语句是：{batch_update_sql}")
            num = cursor.execute(batch_update_sql)
            log.info(f"batch_update 影响的行数是 {num}")
            return num
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"批量更新出错,使用的sql是: {batch_update_sql}")
            raise e
...
def insert_or_update(cursor: Cursor, table_name: str, item: dict, is_only_print_sql: bool = False,
    ...
    if is_only_print_sql:
        log.info(f"使用的插入更新语句是：{insert_or_update_sql}")
    else:
        try:
            log.debug(f"使用的插入更新语句是：{insert_or_update_sql}")
            num = cursor.execute(insert_or_update_sql)
            log.info(f"insert_or_update 影响的行数是 {num}")
            return cursor.lastrowid
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"插入更新出错,使用的sql是: {insert_or_update_sql}")
            raise e
...
def count(cursor, table_name, where, column: list = None, column_str: str = None, distinct: bool = False
    ...
    if is_only_print_sql:
        log.info(f"使用的统计语句是：{count_sql}")
    else:
        try:
            log.debug(f"使用的统计语句是：{count_sql}")
            cursor.execute(count_sql)
            num = cursor.fetchone()[0]
            log.debug(f" {count_sql} 统计的结果是 {num}")
            return num
        except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
            log.error(f"统计出错,使用的sql是: {count_sql}")
            raise e
...
class MySql:
    ...
    def __del__(self):
        # 操作入库
        log.info(f"操作记录如下: {self.operate_record}")
        self.cursor.close()
        self.db.close()
        log.info("释放了数据库连接")
            ...
            if "BatchInsert" == optional:
                try:
                    ...
                    if num:
                        ...
                        log.info(f"批量插入的个数是: {num} 个")
                except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
                    log.error(f"批量插入出现异常,  异常如下:\n  {e} \n 将采用逐条插入或更新的方式插入或更新数据. ")
                    for item in item_list:
                        if item:
                ...
                try:
                    ...
                    if num:
                        ...
                        log.info(f"批量更新影响的行数是: {num} 个")
                except (Exception, RuntimeError, TypeError, NameError, AttributeError, Warning) as e:
                    log.error(f"批量更新出现异常,  异常如下: \n  {e} \n 将采用逐条更新的方式更新数据.")
                    for item in item_list:
                        if item:
    ...
    def batch_update_by_item_list(self, item_list: list, table_name: str, batch_size=100, optional: str = None
        ...
        for item in item_list:
            ...
            if self.current_end > num:
                log.info(f"完成了 第 {num} 个,  共 {num} 个 ")
            else:
                log.info(f"完成了 第 {self.current_end} 个, 共 {num} 个 ")
    ...
    def get_db_data_by_generator(self
        ...
        log.info(f"从数据库中查询到 共有 {total_count} 条数据要处理")
